  ...
  useEffect(() => {
    ...
    function ig(c: HTMLElement, side: "in" | "out", html: string) {
      return new Promise<void>((res) => {
        ...
        if (side === "in") {
          row.innerHTML = `<img class="dm-ig-xav" src="/sophia-avatar.png" alt="Sophie"/><div class="dm-ig-bub in">${html}</div>`;
        } else {
    ...
    /* ── Gmail card ── */
    function gm(c: HTMLElement, isAI: boolean, init: string, bg: string, name: string, sub: string, time: string, body: string) {
      return new Promise<void>((res) => {
        const d = document.createElement("div");
        d.className = `dm-gm-card${isAI ? " ai" : ""}`;
        const av = isAI
          ? `<img class="dm-gm-faceav" src="/sophia-avatar.png" alt="Sophia"/>`
          : `<div class="dm-gm-av" style="background:${bg}">${init}</div>`;
        d.innerHTML = `<div class="dm-gm-head">${av}<div class="dm-gm-hinfo"><div class="dm-gm-name">${name}</div><div class="dm-gm-sub">${sub}</div></div><div class="dm-gm-time">${time}${isAI ? '<span class="dm-ai-tag">✦ AI</span>' : ""}</div></div><div class="dm-gm-body">${body}</div>`;
        c.appendChild(d); c.scrollTop = c.scrollHeight;
        requestAnimationFrame(() => requestAnimationFrame(() => {
          d.classList.add("pop"); setTimeout(res, 300);
        }));
      });
    }
    ...
    async function s1() {
      ...
      await sl(1200);
      const toast = document.getElementById("dm-toast1")!;
      toast.classList.add("show"); await sl(3000); toast.classList.remove("show"); await sl(400);
    ...
    async function s4() {
      ...
      storyRow.innerHTML = `<img class="dm-ig-xav" src="/sophia-avatar.png" alt="S"/><div class="dm-story-card"><img class="dm-story-img" src="/sophia-story.png" alt="story"/><div class="dm-story-lbl">Replied to your story · 🔥🔥🔥</div></div>`;
    ...
    <div className="dm-phone">
      <div className="dm-shell">
        ...
        <div className="dm-scrn">
          <DynIsland/>
          {toastId && (
            <div id={toastId} className="dm-toast">
              <div className="dm-toast-ico">🏦</div>
              <div className="dm-toast-body">
                <div className="dm-toast-app">RBC Royal Bank</div>
                <div className="dm-toast-title">E-Transfer received: $150.00</div>
                <div className="dm-toast-sub">From: Luca M. — Booking deposit</div>
              </div>
  ...
  return (
    <section id="watch" style={{ background: "#080412", position: "relative", zIndex: 2 }}>
      ...
      <SceneWrap id="dm-sc1" pip={0} platform="WhatsApp" PlatIcon={Icon.WA} platformColor="#25d366"
        ...
        <Phone toastId="dm-toast1">
          <div className="dm-screen-wa">
            <IOSBar time="2:14" light/>
            <WaHeader name="Alex" status="online" avBg="#333" avPhoto="/alex-avatar.png"/>
            ...
            <div className="dm-ig-hd">
              ...
              <div className="dm-ig-av-wrap">
                <div className="dm-ig-av-ring">
                  <Image src="/sophia-avatar.png" alt="sophianoir" fill style={{ objectFit: "cover", borderRadius: "50%" }}/>
                </div>
              </div>
              <div>
